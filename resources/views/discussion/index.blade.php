...
@section('title', __('My Discussion Forum'))
@section('content')
<div class="container">
                ...
                <div class="card-body">
                    <div class="row mb-4">
                        <div class="col-md-4 col-sm-5 col-lg-4">
                            <h2 class="card-title">{{ __('Discussions') }}</h2>
                        </div>
                        <div class="col-md-5 col-sm-7 col-lg-6">
                            {{ Form::open([
                                'method' => 'GET', 'action' => 'DiscussionController@search',
                                'role' => 'search',
                            ]) }}
                    ...
                    <table class="table table-bordered table-hover">
                        <thead class="thead-light">
                            ...
                            <th scope="col" class="h6 font-weight-bold">Created By</th>
                        ...
                        <tbody>
                            @foreach ($discussions as $disc)
                                <tr class="table-row" onclick="{{ "document.location = '" . route('discussion.show', ['discussion' => $disc->id]) . "';"}}">
                                    <td>{{ $disc->topic }}</td>
                                    <td>{{ $disc->getCreator() }}</td>
                                    <td>{{ $disc->getNumOfReplies() }}</td>
                                    <td style="font-size: 12px;">{{ $disc->getLastReplyTime() }} by {{ $disc->getLastReplyAuthor() }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
